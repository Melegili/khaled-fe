...
function Calendar(lang, isHijriMode, firstDayOfWeek, isAutoHide, isAutoSelectedDate, year, month, date) {
	...
	Date.lang = lang;
	var self = this,
		thisDate = isHijriMode ? new HijriDate(lang) : new Date(),
		selectedDate = isHijriMode ? new HijriDate(lang) : new Date(),
		...
		yearElement = document.createElement('select'),
	...
	createCalendar =  function() {
		calendarElement.id = "h-g-calendar";
		...
		changeDateModeElement.classList.add('change-lang');
	...
	this.getCurrentDate = function() {
		return isHijriMode ? new HijriDate(lang) : new Date();
	}

	this.getYearWithDiff = function(diffYears) {
	}

	this.toHijri = function(date) {
		var hijriDate = new HijriDate(lang, date.getTime())
		return hijriDate.getFullYear() + '-' + getTwoDigits(hijriDate.getMonth()+1) + '-' + getTwoDigits(hijriDate.getDate());
	};

	this.toGregorian = function(date) {
		var gregorianDate = new Date(date.getTime())
	...
	this.changeDateMode = function() {
		...
		currentDate = thisDate.getDate();
		thisDate.setTime(selectedDate.getTime());
		selectedDate = isHijriMode ? new HijriDate(lang, thisDate.getTime()) : new Date(thisDate.getTime());
		thisDate.setDate(1);
		changeCalendarMode();
		setDateModeAppearance();
		setFirstDayOfWeekAppearance();
	...
	this.getFullDate = function() {
		if(isHijriMode)
			return selectedDate.getFullYear() + '-' + getTwoDigits(selectedDate.getMonth()+1) + '-' +getTwoDigits(selectedDate.getDate());
		else 
			return selectedDate.getFullYear() + '/' + getTwoDigits(selectedDate.getMonth()+1) + '/' +getTwoDigits(selectedDate.getDate());
	}
...
function HijriDate(lang, year, month, date, hour, minute, second, millisecond) {
	...
	this.getMonthName = function(mon) {
		return HijriDate.monthNames[ lang ][(mon === undefined) ? month : parseInt(mon)];
	};
	
	this.getMonthShortName = function(mon) {
		return HijriDate.monthShortNames[ lang ][(mon === undefined) ? month : parseInt(mon)];
	};
	...
	this.getDayName = function(dy) {
		return HijriDate.weekdayNames[ lang ][(dy === undefined) ? day : parseInt(dy)];
	};
	
	this.getDayShortName = function(dy) {
		return HijriDate.weekdayShortNames[ lang ][(dy === undefined) ? day : parseInt(dy)];
	};
...
HijriDate.monthNames = { en: null, ar: null }
	
HijriDate.monthNames.en = ["Muharram", "Safar","Rabi'ul-Awwal","Rabi'ul-Akhir", "Jumadal-Ula","Jumadal-Akhir", "Rajab","Sha'ban", "Ramadan", "Syawwal", "Dhul-Qa'da", "Dhul-Hijja"]
HijriDate.monthNames.ar = ["محرم", "صفر", "ربيع الأول", "ربيع الآخر", "جماد أول", "جماد آخر", "رجب", "شعبان", "رمضان", "شوال", "ذو القعدة", "ذو الحجة"]
	
HijriDate.monthShortNames = { en: null, ar: null }
	
HijriDate.monthShortNames.en = ["Muharram", "Safar","Rabi'ul-Awwal","Rabi'ul-Akhir", "Jumadal-Ula","Jumadal-Akhir", "Rajab","Sha'ban", "Ramadan", "Syawwal", "Dhul-Qa'da", "Dhul-Hijja"]
HijriDate.monthShortNames.ar = ["محرم", "صفر", "ربيع الأول", "ربيع الآخر", "جماد أول", "جماد آخر", "رجب", "شعبان", "رمضان", "شوال", "ذو القعدة", "ذو الحجة"]
	
HijriDate.weekdayNames = { en: null, ar: null }
	
HijriDate.weekdayNames.en = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]
HijriDate.weekdayNames.ar = ["أحد", "اثنين", "ثلاثاء", "أربعاء", "خميس", "جمعة", "سبت"]

HijriDate.weekdayShortNames = { en: null, ar: null }
	
HijriDate.weekdayShortNames.en = ["Sun", "Mon", "Tue", "Wed", "Thur", "Fri", "Sat"]
HijriDate.weekdayShortNames.ar = ["أحد", "اثنين", "ثلاثاء", "أربعاء", "خميس", "جمعة", "سبت"]
...
Date.monthNames = { ar: null, en: null };

Date.monthNames.ar = ['يناير', 'فبراير', 'مارس', 'ابريل', 'مايو', 'يونيو', 'يوليو', 'اغسطس', 'سبتمبر', 'اكتوبر', 'نوفمبر', 'ديسمبر'];
Date.monthNames.en = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];

Date.monthShortNames = { ar: null, en: null };

Date.monthShortNames.ar = ['يناير', 'فبراير', 'مارس', 'ابريل', 'مايو', 'يونيو', 'يوليو', 'اغسطس', 'سبتمبر', 'اكتوبر', 'نوفمبر', 'ديسمبر'];
Date.monthShortNames.en = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];

Date.weekdayNames = { ar: null, en: null };

Date.weekdayNames.en = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday' ];
Date.weekdayNames.ar = ['الأحد', 'الاثنين', 'الثلاثاء', 'الأربعاء', 'الخميس', 'الجمعة', 'السبت' ];

Date.weekdayShortNames = { ar: null, en: null };

Date.weekdayShortNames.ar = ["أحد", "اثنين", "ثلاثاء", "أربعاء", "خميس", "جمعة", "سبت"] 
Date.weekdayShortNames.en = ["Sun", "Mon", "Tue", "Wed", "Thur", "Fri", "Sat"]
...
Date.prototype.getMonthName = function(month) {
	return Date.monthNames[ Date.lang ][(month === undefined) ? this.getMonth() : parseInt(month)];
};

Date.prototype.getMonthShortName = function(month) {
	return Date.monthShortNames[ Date.lang ][(month === undefined) ? this.getMonth() : parseInt(month)];
};

Date.prototype.getDayName = function(day) {
	return Date.weekdayNames[ Date.lang ][(day === undefined) ? this.getDay() : parseInt(day)];
};

Date.prototype.getDayShortName = function(day) {
	return Date.weekdayShortNames[ Date.lang ][(day === undefined) ? this.getDay() : parseInt(day)];
};
